...
public class TossClient {
    ...
    @Value("${payments.toss.secret_key}") String tossSecretKey;
    ...
    private RuntimeException translateError(final HttpClientErrorException ex) {
        if (ex.getStatusCode() == HttpStatus.UNAUTHORIZED) {
            return new IllegalStateException("토스 인증 실패: 시크릿 키를 확인하세요.");
        } else if (ex.getStatusCode() == HttpStatus.NOT_FOUND) {
            return new IllegalStateException("해당 결제 정보가 존재하지 않습니다.");
        }
        return new IllegalStateException("Toss 클라이언트 오류: " + ex.getStatusText());
    }
